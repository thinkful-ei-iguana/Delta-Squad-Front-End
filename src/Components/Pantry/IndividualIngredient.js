...
class IndividualIngredient extends Component {
  ...
  handleSubmit = e => {
    ...
    let updatedIngredient = {
      id: ingredientId,
      ingredient_name: ingredient_name.value || this.props.location.state.ingredient_name,
      in_stock: in_stock.value || this.props.location.state.in_stock,
      notes: notes.value || this.props.location.state.notes
    }
    console.log('updated ingredient to be sent to server is', updatedIngredient);
    fetch(url, {
      method: 'PATCH',
      headers: {
